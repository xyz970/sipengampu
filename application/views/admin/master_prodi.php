        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
              ...
              <button class="btn btn-success btn-sm ml-auto" onclick="add()"><i class="fa fa-plus"></i> Tambah Prodi</button>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
...
<script>
  ...
  var add = function() {
    let modal = $('#modal')
    modal.find('.modal-title').html('Tambah data prodi')
    modal.modal('show')
  ...
  var showUpdate = (id) => {
    let modal = $('#modal')
    modal.find('.modal-title').html('Perbarui data prodi')
    modal.modal('show')
    let form = modal.find('form')
    form[0].reset()
    $.get('<?= site_url('api/prodi/') ?>'+id ,(res) => {
      if(res.success) {
